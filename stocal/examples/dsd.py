...
re_reversible_double = re.compile('(?:\[\W*?(\w)(?:\^\W*?\]))')  # Matches on double toeholds of the form [A^] not [A^ B]
re_double_lab = re.compile('(\w)(?=\^)(?=[^<>{}]*])')  # Returns the label of a double toehold regex. Better way of doing this?
...
#re_gate = re.compile('f"({re_upper2})?({re_lower2})?{re_double}({re_upper2})?({re_lower2})?"')
            ...
            for match_2 in re.finditer(regex_2, l):
                if match_1.group() == match_2.group():
                    if regex_1 == re_upper_lab:
                        print("upper_th")
                        part_a = k[:match_1.start()] + re.search(re_closing, k[match_1.start():]).group() + l[:match_2.start()] + re.search(
                        ...
                        print("lower_th")
                        part_a = k[:match_1.start()] + re.search(re_closing, k[match_1.start():]).group() + l[:match_2.start()] + re.search(re_closing, l[match_2.start():]).group()
...
class UnbindingRule(stocal.TransitionRule):
    ...
    def toehold_unbinding(self, kl):
        ...
        for double_th in re.finditer(re_reversible_double, kl):
            print("kl:", kl)
            label = re.search(re_double_lab, double_th.group()).group()  # Retrieve the label of the toehold we are unbinding
            prefix, suffix = "", ""
...
if __name__ == '__main__':
    ...
    initial_state = {"<A>{B}[D^]<C^>{C^*}:{C^*}<C>[A]<F^>{F^*}": 60}
